...
def initialize():
    ...
    global side
    side = max(
        int(heights[0] // 100 * 1.5), 30
    )  # generates side length by scaling with respect to monitor vertical resolution
    global border
    border = int(max(side / 10, 1))  # the borderwidth of each block
    global delay
    delay = int(
        input(
            "Enter the delay of the pointer in milliseconds (-1 for default (10ms)): "
        )
    )  # the delay of the "head" of the algorithm
    if delay == -1:  # defaults to 10 milliseconds
        delay = 10
    global gridSize
    gridSize = int(input("Enter Grid Size: "))  # gets user to input grid size (n * n)
    while (
        gridSize < 2 or coordinateOffset(gridSize) + heights[0] / 1920 * 80 > heights[0]
    ):  # checks if grid will fit in screen or if grid is 1 * 1 or lower
        print("Entered input is invalid. ")
        gridSize = int(input("Enter Grid Size: "))
    global consoleSize
    consoleSize = coordinateOffset(
        gridSize
    )  # gets console pixel size based on grid size
    global blockColor
    blockColor = colorInput(
        "Enter block color (Enter p to print all available colors): "
    )  # gets block colors
    global borderColor
    borderColor = colorInput(
        "Enter border color (Enter p to print all available colors): "
    )  # gets border colors
    global trackerColor
    trackerColor = colorInput(
        "Enter tracker color (Enter p to print all available colors): "
    )  # gets color for "head" of the algorithm
...
class Maze:
    ...
    def __init__(self, _gridSize, _coordinates):
        ...
        self._stack, self._solutionStack = (
            deque(),
            deque(),
        )  # uses deque (double ended queue) as a stack data structure
        self._visited = full(
            (self._gridSize, self._gridSize), False
        )  # stores points that have been visited
        self._stack.append(self._coordinates[0])  # adds start coordinate to stack
        self._solutionStack.append(
            self._coordinates[0]
        )  # adds start coordinate to solution stack
        self._visited[self._coordinates[0][0]][
            self._coordinates[0][1]
        ] = True  # marks start coordinate as visited
    ...
    def _clearPath(first, second, color, offset=0):
        start = Rect(
            coordinateOffset(first[0]) + offset,
            coordinateOffset(first[1]) + offset,
            side - 2 * offset,
            side - 2 * offset,
        )
        end = Rect(
            coordinateOffset(second[0]) + offset,
            coordinateOffset(second[1]) + offset,
            side - 2 * offset,
            side - 2 * offset,
        )
    ...
    def _updateTracker(first, second):
        pygame.draw.rect(
            console,
            blockColor,
            (coordinateOffset(first[0]), coordinateOffset(first[1]), side, side),
        )  # covers old coordinate with block color
        pygame.draw.rect(
            console,
            trackerColor,
            (coordinateOffset(second[0]), coordinateOffset(second[1]), side, side),
        )  # sets new coordinate as tracker color
    ...
    def _moveTo(self, _pair):
        ...
        self._clearPath(
            top, _pair, blockColor
        )  # clears path from old coordinate to new coordinate
        self._updateTracker(top, _pair)  # sets new coordinate as head/tracker
        pygame.display.set_caption(
            "APCSP Create - Maze Generation Status: "
            + "%0.2f" % (self._visitedCount / self._gridArea * 100)
            + "%"
    ...
    def visitNeighbors(self):
        pygame.display.set_caption(
            "APCSP Create - Maze Generation Status: 0.00%"
        )  # sets caption
        while (
            self._visitedCount != self._gridArea
        ):  # while every coordinate hasn't been visited
            ...
            if len(neighbors) != 0:  # checks if there is at least one neighbor
                ...
                self._moveTo(
                    randNeighbor
                )  # moves current coordinate to be the new coordinate
                if (
                    self._solutionStack[-1] != self._coordinates[1]
                ):  # if solution hasn't been found yet, new coordinate will be a part of the solution
                ...
                self._updateTracker(
                    top, self._stack[-1]
                )  # sets head/trackker as last coordinate
                if (
                    self._solutionStack[-1] != self._coordinates[1]
                ):  # if solution hasn't been found yet, coordinate has to be removed
    ...
    def traverseSolution(self):
        pygame.draw.rect(
            ...
            blockColor,
            (
                coordinateOffset(self._stack[-1][0]),
                coordinateOffset(self._stack[-1][1]),
                side,
                side,
            ),
        )  # removes old tracker
        pygame.display.update()
        pygame.display.set_caption(
            "APCSP Create - Maze Solution Status: 0.00%"
        )  # updates window caption
        originalSize = len(
            self._solutionStack
        )  # gets amount of coordinates in solution so that percent completed can be generated
        ...
        if originalSize == 1:  # if start coordinate and end coordinate were the same
            pygame.draw.rect(
                ...
                trackerColor,
                (
                    coordinateOffset(self._solutionStack[-1][0]) + offset,
                    coordinateOffset(self._solutionStack[-1][1]) + offset,
                    side - 2 * offset,
                    side - 2 * offset,
                ),
            )
            ...
            pygame.display.set_caption(
                "APCSP Create - Maze Solution Status: "
                + "%0.2f" % ((1 - (len(self._solutionStack)) / originalSize) * 100)
                + "%"
            )  # updates window caption
            pygame.display.update()
        ...
        while len(self._solutionStack) > 0:
            ...
            self._solutionStack.pop()  # removes top of stack
            self._clearPath(
                top, self._solutionStack[-1], trackerColor, offset
            )  # draws path from old coordinate to new coordinate
            if (
                firstCoordinate is False or len(self._solutionStack) == 1
            ):  # if element is first or last element
                ...
                pygame.draw.rect(
                    console,
                    trackerColor,
                    (coordinateOffset(top[0]), coordinateOffset(top[1]), side, side),
                )
                pygame.draw.rect(
                    console,
                    blockColor,
                    (
                        coordinateOffset(top[0]) + offset,
                        coordinateOffset(top[1]) + offset,
                        side - 2 * offset,
                        side - 2 * offset,
                    ),
                )
...
def main():
    ...
    pygame.draw.rect(
        console,
        borderColor,
        (0, 0, coordinateOffset(gridSize), coordinateOffset(gridSize)),
    )  # fills screen with border color
    for i in range(gridSize):
        for j in range(gridSize):
            pygame.draw.rect(
                console,
                blockColor,
                (coordinateOffset(i), coordinateOffset(j), side, side),
            )  # adds blocks on top of border color
    ...
    for i in range(2):
        ...
        while finding is False:
            ...
            for action in pygame.event.get():
                if (
                    action.type == pygame.QUIT
                ):  # checks if user clicks red x to exit, and exits if so
                    pygame.quit()
                    sys.exit()
                elif (
                    action.type == MOUSEBUTTONDOWN
                ):  # checks if user clicked a point, and takes the floor of the x and y, so that the top right of the block is still considered part of the block
    ...
    )  # initializes object with grid size and coordinates
